...
export function ValidationSummaryBar({
  ...
  return (
    <div className="glass grid gap-4 rounded-2xl p-4 sm:grid-cols-2 lg:grid-cols-4 lg:p-5">
      <Stat label="Documents analysés" value={analyzedDocumentsCount} accent="text-zinc-100" />
      ...
      />
      <Stat label="Déjà validés" value={validatedCount} accent="text-emerald-400/80" />
      ...
      {highConfidenceCount > 0 && pendingCount > 0 && (
        <div className="flex items-center sm:col-span-2 lg:col-span-4">
          <button
            ...
            className="w-full rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-2.5 text-sm font-medium text-emerald-400 transition-colors hover:bg-emerald-500/20 sm:w-auto"
          >
