...
export type ChargesHeroUploadState = "idle" | "uploaded";

type ChargesHeroProps = {
  onFiles: (files: File[]) => void;
  disabled?: boolean;
  uploadState?: ChargesHeroUploadState;
  uploadedFileName?: string;
  uploadedCount?: number;
  detectedCategoryCount?: number;
};

const HERO_BADGE = "Charges déductibles";
const HERO_TITLE = "Ajoutez vos dépenses liées au bien";
const HERO_EXPLANATION =
  "Déposez vos factures et justificatifs.\nL'IA détecte automatiquement les charges et prépare les informations nécessaires pour votre déclaration.\nVous pourrez corriger ou compléter les informations ensuite.";
const UPLOAD_PROMPT = "Glissez vos documents ici ou cliquez pour importer";
const UPLOAD_HINT =
  "Taxe foncière, assurances, copropriété, EDF, CFE, gestion — PDF, images, captures — dépôt multiple accepté";

export function ChargesHero({
  onFiles,
  disabled = false,
  uploadState = "idle",
  uploadedFileName,
...
function UploadedSummary({
  ...
  const analysisLabel =
    categoryCount && categoryCount > 0
      ? `${categoryCount} catégorie${categoryCount > 1 ? "s" : ""} en cours d'analyse`
      : "Classification des charges en cours";
