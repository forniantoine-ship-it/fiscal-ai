...
import { YearBadge } from "@/components/lmnp/dashboard/DashboardHero";
...
export type ActiviteHeroUploadState = "idle" | "uploaded";

type ActiviteHeroProps = {
  year: number;
  saveLabel?: string | null;
  saveActive?: boolean;
  onFiles: (files: File[]) => void;
  disabled?: boolean;
  uploadState?: ActiviteHeroUploadState;
  uploadedFileName?: string;
  uploadedCount?: number;
};

const HERO_TITLE = "Ajoutez votre document INPI";
const HERO_EXPLANATION =
  "Déposez votre extrait INPI — l'IA extrait les informations clés. Vérifiez avant validation.";
const UPLOAD_PROMPT = "Glissez votre document ici ou cliquez pour importer";

export function ActiviteHero({
  year,
  saveLabel,
  saveActive,
  onFiles,
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-wrap items-center justify-center gap-2">
        ...
        >
          Déclaration LMNP {year + 1}
        </span>
        <YearBadge year={year} />
        {isUploaded ? <SuccessPill /> : null}
        {!isUploaded && saveLabel ? (
          <SavePill label={saveLabel} active={saveActive ?? false} />
        ) : null}
      </div>
      ...
      <p
        className="relative mx-auto mt-2.5 max-w-lg"
        style={{
...
function UploadedSummary({
  ...
  const label =
    count === 1 ? "1 document importé" : `${count} documents importés`;
