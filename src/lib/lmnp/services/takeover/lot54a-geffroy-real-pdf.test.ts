...
// Colonnes : Amort. début | Amort. fin (clôture 31/12/2023).
...
const PAGE2_ROWS: DepreciationRegisterPdfRow[] = [
  ...
  // Totaux documentaires imprimés (page 3 réelle du PDF) — Amort. début / Amort. fin
  {
    rowType: "total",
    pageNumber: 2,
    scopeLabel: "Total",
...
describe("Lot 5.4-A — GEFFROY FRERES (PDF réel) — extraction same-year (registre 2023)", () => {
  it("PARSER CONTRACT (Vision scriptée) — comptes PCG de section propagent pcgAccountCode", async () => {
    ...
    // Same-year : sourceFiscalYear omis → Amort. début → cumulOuverture (pas preuve Opening N).
...
describe("Lot 5.4-A — GEFFROY reprise N←N-1 (registre 2023 → ouverture 2024)", () => {
  it("Amort. fin → cumulOuverture ; C30300/C31000/C40200 non bloqués par Amort. début vide ; B80400 reste missing", async () => {
    const result = await extractDepreciationRegisterFromPdf({
      file: loadGeffroyFile(),
      documentId: DOC,
    ...
    assert.ok(c30300 && isCandidatePresent(c30300.cumulOuverture));
    assert.equal(c30300!.cumulOuverture.value, 78.37);
    assert.equal(c30300!.cumulOuverture.provenance.fieldLabel, "closingCumulative");

    const c31000 = byRef("C31000");
    assert.ok(c31000 && isCandidatePresent(c31000.cumulOuverture));
    assert.equal(c31000!.cumulOuverture.value, 32.97);

    const c40200 = byRef("C40200");
    assert.ok(c40200 && isCandidatePresent(c40200.cumulOuverture));
    assert.equal(c40200!.cumulOuverture.value, 101.61);

    const b80400 = byRef("B80400");
    assert.ok(b80400);
    assert.ok(isCandidateAbsent(b80400!.cumulOuverture));
    assert.ok(isCandidatePresent(b80400!.coutBrut));
    if (isCandidatePresent(b80400!.coutBrut)) assert.equal(b80400!.coutBrut.value, 1559.91);

    // Amort. début ≠ ouverture N (ex. B80700 : début 470,81 ≠ fin 529,00)
    const b80700 = byRef("B80700");
    assert.ok(b80700 && isCandidatePresent(b80700.cumulOuverture));
    assert.equal(b80700!.cumulOuverture.value, 529);
    assert.notEqual(b80700!.cumulOuverture.value, 470.81);

    assert.ok(
      result.diagnostics.some((d) => d.code === "SOURCE_OPENING_CUMULATIVE_IGNORED"),
      "Amort. début N-1 doit être signalé comme non mappé vers ouverture N",
    );
    assert.ok(
      !result.diagnostics.some((d) => d.code === "CLOSING_CUMULATIVE_IGNORED"),
      "Amort. fin ne doit plus être CLOSING_CUMULATIVE_IGNORED en reprise N←N-1",
    );
  });
});
