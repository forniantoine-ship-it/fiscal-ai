...
| 372 | Résultat fiscal après imputation — Déficit (col.2) | `resultatFiscal` (si **<0**) — **ne reçoit plus jamais `deficitNouveau` (correction `aa765cb`)** | absente (F-006 garantit `resultatFiscal`≥0, TRF-0031 — jamais exercée en pratique) | 1 | x=504.57 (droite, A4), y=787.8 | eur-arrondi | **GEO_CONFIRMED** (`tests/position-oracle.test.ts`, fixture synthétique) | NON CONTESTÉ — le déficit LMNP du dossier témoin (9862) est désormais porté par la case 330, pas ici |
